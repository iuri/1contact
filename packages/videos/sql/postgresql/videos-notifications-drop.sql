...
begin
    for row in select nt.type_id
               ...
               where nt.short_name in (''videos_video_notif'', ''video_item_notif'')
    loop
...
begin
        ...
        impl_id := acs_sc_impl__get_id (
                      ...
                      ''videos_video_notif_type''	-- impl_name
        );
	...
	select into v_foo type_id 
	 ...
	 where sc_impl_id = impl_id
	  and short_name = ''videos_video_notif'';
	...
	select into v_foo type_id 
	  from notification_types
	 where sc_impl_id = impl_id
	   and short_name = ''video_item_notif'';

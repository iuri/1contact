...
if { $current_page_group >= 1 } {
    ...
    set url_previous_group [export_vars -base search {{q $urlencoded_query} search_package_id offset num __csrf_token}]
} else {
...
if { $current_page_group < $last_page_group } {
    set offset [expr {($current_page_group + 1) * $pages_per_group * $limit}]
    set url_next_group [export_vars -base search {{q $urlencoded_query} search_package_id offset num __csrf_token}]
} else {
    set url_next_group ""
}
...
if { $current_result_page > 0 } { 
    ...
    set url_previous [export_vars -base search {{q $urlencoded_query} search_package_id offset num __csrf_token}]
} else {
    set url_previous ""
}
...
if { $current_result_page < $last_result_page } { 
    ...
    set url_next [export_vars -base search {{q $urlencoded_query} search_package_id offset num __csrf_token}]
} else {
    set url_next ""
}
...
for { set __i $from_result_page } { $__i <= $to_result_page} { incr __i } {
    set link [export_vars -base search {{q $urlencoded_query} search_package_id __csrf_token}]
